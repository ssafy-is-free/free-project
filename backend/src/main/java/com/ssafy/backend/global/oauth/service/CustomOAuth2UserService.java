...
import com.ssafy.backend.domain.user.repository.RankRepository;
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        System.out.println(oAuth2User.getAttributes().get("login")); //유저 아이디(닉네임)
        System.out.println(oAuth2User.getAttributes().get("avatar_url")); //프로필 사진.
    ...
    public OAuth2User processOAuthUser(OAuth2UserRequest oAuth2UserRequest,OAuth2User oAuth2User)throws OAuth2AuthenticationProcessingException{

        //어떤 OAuthProivder인지 판단
        OAuth2UserInfo oAuth2UserInfo = customOAuthUserInfoFactory.getOAuth2UserInfo(oAuth2UserRequest.getClientRegistration().getRegistrationId(), oAuth2User.getAttributes());

        //열어봤을때 아이디가 없으면 예외 던짐.
        if(StringUtils.isEmpty(oAuth2UserInfo.getName())){
            throw new OAuth2AuthenticationProcessingException(oAuth2UserRequest.getClientRegistration().getRegistrationId() + "에서 ID를 찾을 수 없습니다.");
        }
    ...
    public User registerUser(String nickname, String image){
        ...
        User user = User.create(nickname, image, saveRank);
