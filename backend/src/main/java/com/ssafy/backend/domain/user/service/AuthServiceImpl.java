...
public class AuthServiceImpl implements AuthService{
    ...
    @Override
    public AccessTokenResponseDTO reissueToken(String oldAccessToken, String refreshToken) {
        ...
        if(user.getRefreshToken() == null || !tokenProvider.validateRefreshToken(refreshToken, user.getRefreshToken())) throw new CustomException(NOT_INVALID_REFRESH_TOKEN);

        //토큰 생성후, DTO변환해서 리턴
        return AccessTokenResponseDTO.createDTO(tokenProvider.createAccessToken(UserPrincipal.createUserDetails(user)));
    }
}
