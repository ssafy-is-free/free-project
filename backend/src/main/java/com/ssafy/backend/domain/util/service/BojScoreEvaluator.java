...
public class BojScoreEvaluator {
	public static int scoreEvaluator(Baekjoon baekjoon) {
		...
		int score = (int)Math.ceil(
			((double)baekjoon.getFailCount() / baekjoon.getPassCount()) * baekjoon.getSubmitCount()
				- baekjoon.getTryFailCount() * (getTierNumberFromUrl(baekjoon.getTier())
				/ (double)baekjoon.getPassCount())
				* 0.5);
